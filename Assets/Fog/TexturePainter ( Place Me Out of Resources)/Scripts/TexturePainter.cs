...
public class TexturePainter : MonoBehaviour {
    ...
    [Range(0,5)]
	public float brushSize=2.0f; //The size of our brush
	...
	void DoAction(){	
		...
		if(HitTestUVPosition(ref uvWorldPosition)){
			...
			if(mode==Painter_BrushMode.PAINT){
				...
				brushObj.GetComponent<SpriteRenderer>().color=Color.black; //Set the brush color
			}
			else{
				brushObj=(GameObject)Instantiate(Resources.Load("TexturePainter-Instances/DecalEntity")); //Paint a decal
			}
			brushColor.a=brushSize*50; // Brushes have alpha to have a merging effect when painted over.
